...
def print_recommendations(user_data):
    ...
    response = requests.get(
        f"{hostname}/api/matching/relation-recommendations",
    ...
    table = Table(
        title=f'{user_data[0]["firstname"]} {user_data[0]["lastname"]}\'s recommendations',
        box=box.ROUNDED,
    )
    ...
    for dict in data:
        mentorAccount = get_data(
            'SELECT * FROM "user" INNER JOIN account ON ("user".accountID = account.accountID) WHERE "user".userID=%s',
            (dict["userID"],),
        )
        mentorTopics = get_data(
            'SELECT * FROM "user_topic" WHERE userID=%s', (dict["userID"],)
        )
        mentorRatings = get_data(
            'SELECT * FROM "user_rating" WHERE userID=%s', (dict["userID"],)
        )
        topics = []
        ...
        table.add_row(
            f"{mentorAccount[0]['firstname']} {mentorAccount[0]['lastname']}",
            str(dict["userID"]),
            str(topics),
            str(ratings),
            str(dict["compatibility"]),
        )
