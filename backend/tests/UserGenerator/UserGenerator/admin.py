...
def add_skill(skill_name):
    ...
    response = requests.post(
        f"{hostname}/api/admin/add-skill",
...
def add_random_skills(skill_count=None):
    ...
    with Progress() as progress:
        skill_progress = progress.add_task(
            "[cyan]Adding skills...[/]", total=skill_count
        )
...
def add_topic(topic_name):
    ...
    response = requests.post(
        f"{hostname}/api/admin/add-topic",
...
def add_random_topics(topic_count=None):
    ...
    start = perf_counter()
    with Progress() as progress:
        topic_progress = progress.add_task(
            "[cyan]Adding topics...[/]", total=topic_count
        )
...
def add_area(area_name):
    ...
    response = requests.post(
        f"{hostname}/api/admin/add-business-area",
...
def add_random_areas(area_count=None):
    ...
    start = perf_counter()
    with Progress() as progress:
        area_progress = progress.add_task(
            "[cyan]Adding business areas...[/]", total=area_count
        )
