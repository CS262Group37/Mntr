...
def add_plan(relationID, title, description):
    response = requests.post(
        f"{hostname}/api/plan/add-plan",
        data={"relationID": relationID, "title": title, "description": description},
        timeout=10,
...
def add_random_plans(plan_count=None):
    ...
    if not relations:
        console.print(
            "[red]The system does not contain enough relations to add any plans[/]"
        )
        return

    start = perf_counter()
    with Progress() as progress:
        plan_progress = progress.add_task(
            "[cyan]Adding random plans of action...[/]", total=plan_count
        )
        added_plans = 0
        for i in range(plan_count):
            ...
            mentee_data = get_data(
                ...
                (random_relation["menteeid"],),
            )

            # Login as the mentee
            if not authentication.login_user(
                mentee_data[0]["email"],
                mentee_data[0]["password"],
                mentee_data[0]["role"],
            ):
                continue
