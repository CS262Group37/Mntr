...
def create_account(first_name, last_name, email, password, profilePicture=None):
    response = requests.post(
        f"{hostname}/api/auth/register-account",
        data={
            "email": email,
            "password": password,
...
def create_user(
    role, businessArea=None, topics=None, skills=None, ratings=None, adminPassword=None
):

    if role == "admin":
        data = {"role": role, "adminPassword": adminPassword}
    elif role == "mentor":
        data = {
            "role": role,
    ...
    response = requests.post(
        f"{hostname}/api/auth/register-user",
        data,
...
def create_random_accounts_and_users(account_count=None):
    ...
    businessAreas = remove_tuples(
        database.get_data('SELECT "name" FROM system_business_area')
    )
    ...
    if not businessAreas or not skills or not topics:
        console.print(
            "[red]System is missing either business areas, skills or topics[/]"
        )
        return

    with Progress() as progress:
        account_progress = progress.add_task(
            "[cyan]Adding random accounts and users...[/]", total=account_count
        )
        ...
        for i in range(account_count):
            ...
            if create_account(
                ...
                if random_role < 5:
                    if create_user(
                        "mentee",
                        businessArea=choice(businessAreas),
                        topics=sample(topics, k=randrange(1, len(topics))),
                        skills=skills,
                        ratings=random_ratings(skills),
                    ):
                        created_users += 1
                elif random_role < 10:
                    if create_user(
