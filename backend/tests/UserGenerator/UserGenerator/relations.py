...
def create_relation(menteeID, mentorID):
    # Get mentee data
    mentee_data = get_data(
        'SELECT * FROM "user" INNER JOIN account ON ("user".accountID = account.accountID) WHERE "user".userID=%s',
        (menteeID,),
    )
    if not mentee_data:
        return False
    ...
    # Login as the mentee
    if not authentication.login_user(
        mentee_data[0]["email"], mentee_data[0]["password"], mentee_data[0]["role"]
    ):
        return False

    response = requests.post(
        f"{hostname}/api/relations/create-relation",
...
def add_random_relations(relation_count=None):
    ...
    if not mentees or not mentors:
        console.print(
            "[red]The system does not contain enough users to make any relations[/]"
        )
        return
    ...
    with Progress() as progress:
        relation_progress = progress.add_task(
            "[cyan]Adding random relations...[/]", total=relation_count
        )
        added_relations = 0
...
def print_all_relations():
    ...
    for row in relations:
        table.add_row(
            str(row["relationid"]), str(row["menteeid"]), str(row["mentorid"])
        )
...
def print_relations(user_data):
    if user_data[0]["role"] == "mentee":
        relations = get_data(
            'SELECT * FROM relation INNER JOIN "user" ON relation.mentorID = "user".userID INNER JOIN account ON account.accountID = "user".accountID WHERE relation.menteeID = %s',
            (user_data[0]["userid"],),
        )
    else:
        relations = get_data(
            'SELECT * FROM relation INNER JOIN "user" ON relation.menteeID = "user".userID INNER JOIN account ON account.accountID = "user".accountID WHERE relation.mentorID = %s',
            (user_data[0]["userid"],),
        )

    table = Table(
        title=f'{user_data[0]["firstname"]} {user_data[0]["lastname"]}\'s relations',
        box=box.ROUNDED,
    )
    table.add_column("RelationID", justify="center")
